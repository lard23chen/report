...
> **Sheets 欄位結構**（gid=1883089838，2026/08/18 實測確認）：
> `col0=監控日期 | col1=監控時間 | col2=節目名稱 | col3=監控等級 | col4=機器等級 | col5=PM | col6=A系統+會員 | col7=金流 | col8=D系統 | col9=Q系統 | col10=E系統 | col11=備註`
> col4 為實際機器大小（小/中/大/X），X 表示無機器（計入 total 但不計入小/中/大）；col10 非空且非 `X` 才算 `hasE: true`。
> 節目名稱可能含換行（quoted field），取第一行即可。
...
*2026/08/18：更新至 2026/07 月度資料（總費用 $1,767,384，▼0.6%）；同步 2026 機器等級統計至 Google Sheets 最新內容（7月 24→33 場、8月 3→26 場、新增 9月 2 場）；AWS 期間（2025/11 起共 9 個月）月均 $462,076，較 Azure 期間（2024/11–2025/10）月均 $667,312 下降 30.8%*
...
*最後更新日期：2026/08/18（新增 2026/07 資料與機器等級統計同步；附錄 A 快照至 2026/07）*
