...
async function generateReport() {
    try {
        ...
        const TARGET = '2025 7-ELEVEN高雄啤酒音樂節';
        const TARGET_FIELD = '節目/商品名稱';

        console.log(`Fetching data for: ${TARGET}...`);
        const data = await db.collection('Qware_Ticket_Data').find({ [TARGET_FIELD]: TARGET }).toArray();
...
<head>
    ...
    <title>2025 7-ELEVEN 高雄啤酒音樂節 - 專案分析報表</title>
    ...
    <style>
        ...
        .rain-badge { display: inline-block; background: rgba(239,68,68,0.15); color: #ef4444; border: 1px solid rgba(239,68,68,0.3); padding: 3px 10px; border-radius: 20px; font-size: 0.75em; margin-left: 8px; font-weight: 600; }
    </style>
</head>
<body>
<div class="container">
    <header>
        <div>
            <div class="logo-text">2025 高雄啤酒音樂節</div>
            <div class="logo-sub">7-ELEVEN 高雄啤酒音樂節 專案銷售分析報表</div>
        </div>
        <div class="meta">報表生成: ${reportTime}<br>銷售紀錄: ${data.length.toLocaleString()} 筆<br>節目代碼: B08SCWCO</div>
    </header>
    ...
    <!-- Refund Reason -->
    <div class="chart-card" style="min-height:0">
        <h3>退票原因分析 <span class="rain-badge">⛈ 含因雨退票</span></h3>
        <table id="refundTable"><thead><tr><th>退票原因</th><th class="text-right">張數</th><th class="text-right">退票金額</th><th class="text-right">占退票比</th></tr></thead><tbody></tbody></table>
    </div>
    ...
    <!-- Peak Analysis -->
    <div class="chart-card" style="min-height:0">
        <h3>開賣尖峰時段分析 (2025-05-23 12:00 – 13:00)</h3>
        <div class="peak-table-container">
            <table id="peakTable">
                <thead><tr><th>時間</th><th class="text-right">當分鐘張數</th><th class="text-right">累計張數</th><th class="text-right">信用卡</th><th class="text-right">ATM/ibon</th><th class="text-right">累計轉換率</th></tr></thead>
                <tbody></tbody>
            </table>
        </div>
    </div>
...
function init() {
    ...
    showDates.forEach(showDate => {
        ...
        // Check if this date has rain refunds
        const rainRefunds = sr.filter(o => (o['退票因素'] || '').includes('雨')).length;
        const rainBadge = rainRefunds > 0 ? ' <span style="background:rgba(239,68,68,0.15);color:#ef4444;padding:2px 8px;border-radius:20px;font-size:0.75em;">⛈ 因雨退票 ' + rainRefunds + '張</span>' : '';
        const tr = document.createElement('tr');
        tr.innerHTML = \`<td style="font-weight:600; color:var(--accent-color); white-space:nowrap;">\${timeLabel}\${rainBadge}</td>
            ...
            <td class="text-right">\${fmtMoney(sRefFees)}\`;
    ...
    // Show Cards (2025-07-04 Fri, 2025-07-05 Sat, 2025-07-06 Sun)
    const shows = [
        { date: '2025-07-04', label: '7/4 (五) 15:00 Day 1' },
        { date: '2025-07-05', label: '7/5 (六) 15:00 Day 2' },
        { date: '2025-07-06', label: '7/6 (日) 15:00 Day 3' }
    ];
    const showContainer = document.getElementById('showCards');
    shows.forEach(s => {
        const cnt = valid.filter(d => d['演出時間/規格'] && d['演出時間/規格'].includes(s.date)).length;
        const allCnt = dbData.filter(d => d['演出時間/規格'] && d['演出時間/規格'].includes(s.date)).length;
        const refCnt = refund.filter(d => d['演出時間/規格'] && d['演出時間/規格'].includes(s.date)).length;
        const rainCnt = refund.filter(d => d['演出時間/規格'] && d['演出時間/規格'].includes(s.date) && (d['退票因素']||'').includes('雨')).length;
        const div = document.createElement('div');
        div.className = 'card';
        const rainNote = rainCnt > 0 ? \`<div class="sub" style="color:#ef4444;">⛈ 因雨退票 \${rainCnt} 張</div>\` : '';
        div.innerHTML = \`<h3>\${s.label}</h3><div class="value">\${cnt.toLocaleString()} 張</div><div class="sub">\${pct(cnt, valid.length)} 占比</div>\${rainNote}<div class="progress-bar"><div class="progress-fill" style="width:\${valid.length > 0 ? cnt/valid.length*100 : 0}%"></div></div>\`;
    ...
    const sessionShows = [
        { date: '2025-07-04', label: '7/4 (五) Day 1', color: '#38bdf8' },
        { date: '2025-07-05', label: '7/5 (六) Day 2', color: '#818cf8' },
        { date: '2025-07-06', label: '7/6 (日) Day 3', color: '#fb7185' }
    ];
    ...
    refund.forEach(o => {
        const reason = o['退票因素'] || '未填寫';
        refundReasonStats[reason] = refundReasonStats[reason] || { c: 0, amt: 0 };
        refundReasonStats[reason].c++;
        refundReasonStats[reason].amt += (o['實退金額'] || 0);
    });
    const refBody = document.querySelector('#refundTable tbody');
    const totalRefund = refund.length;
    Object.entries(refundReasonStats).sort((a,b)=>b[1].c-a[1].c).forEach(([k,v]) => {
        const tr = document.createElement('tr');
        const isRain = k.includes('雨');
        tr.innerHTML = \`<td style="\${isRain ? 'color:#ef4444;font-weight:600;' : ''}">\${isRain ? '⛈ ' : ''}\${k}</td>
            <td class="text-right" style="\${isRain ? 'color:#ef4444;font-weight:600;' : ''}">\${v.c.toLocaleString()}</td>
            <td class="text-right" style="\${isRain ? 'color:#ef4444;font-weight:600;' : ''}">\${fmtMoney(v.amt)}</td>
            <td class="text-right">\${pct(v.c, totalRefund)}</td>\`;
    ...
    // Peak Analysis - 2025-05-23 12:00-13:00
    const peakDate = '2025-05-23';
    const peakStats = {};
    for (let m = 0; m < 60; m++) {
...
<\/script>
        ...
        const fileName = 'A_KaohsiungBeerFestival_2025.html';
