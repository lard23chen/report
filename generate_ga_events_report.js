    ...
    try {
        ...
        for (let event of topEvents) {
            ...
            evSessions.forEach(s => {
                let dts = s.CreateTime ? String(s.CreateTime) : '';
                let timeKey = dts.slice(0, 16); // 'YYYY-MM-DD HH:mm'
                if (!timeKey) return;
            ...
            evReads.forEach(r => {
                let dtr = r.CreateTime ? String(r.CreateTime) : '';
                let timeKey = dtr.slice(0, 16);
                if (!timeKey) return;
            ...
            clientData.push({
                ...
                start: timeDataArr.length > 0 ? timeDataArr[0].time : '',
                end: timeDataArr.length > 0 ? timeDataArr[timeDataArr.length - 1].time : '',
                data: timeDataArr
            });
        }
...
<script>
    ...
    function updateDashboard(idx) {
        ...
        const activeDArr = d.data.map(item => item.activeD);
        const activeAArr = d.data.map(item => item.activeA);
