...
- **系統文件（HTML）：`F_MEM_BlackList_System_Doc.html`**（2026/08/18 新增）——給營運／IT 閱讀的系統說明頁，涵蓋系統概述、架構與資料流、資料來源、內嵌資料結構、功能、判讀門檻、維運作業、已知風險、欄位值對照。與本 MD 的分工：**本 MD 是開發用規範＋完整異動沿革，系統文件是現況快照**，兩者需同步維護（改門檻／時間窗／欄位時，兩邊都要更新）。已隨 `daily_update.bat` 的 `SYNC_FILES` 自動同步至 NewReport 站台。
