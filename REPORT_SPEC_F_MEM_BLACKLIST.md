...
| 資料範圍 | 黑名單詳情（BLACKLIST_DATA）：**僅嵌入近 30 天內** `UPDATE_TIME` 且 `MEMO0='Y'` 的資料（見 §2）；電話搜尋索引（ALL_INDEX，2026/08/05 新增）：**涵蓋全部會員**，不分黑名單狀態（見 §2.2）；IP 關聯：**僅嵌入近 7 天登入紀錄**，每組 IP 最多列前 30 個共用帳號（見 §2.1） |
| 負責人 | 陳俊良 |
...
```js
{ total: 15138, minDate: "2026-07-06", maxDate: "2026-08-04", ipWindowDays: 7, ipLinkCap: 30, allIndexTotal: 403350 }
```
