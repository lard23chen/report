...
```js
{
  ...
  wt:     "MAJOR",                  // （2026/08/06 新增，選填）同門號對應 5+ 個不同 USER_ID 時才有此欄位，見 §4.6
}
```
...
[
  {
    ...
    wt: "MAJOR",                   // （2026/08/06 新增，選填）見 §4.6
  },
  // … 全部 40 萬餘筆會員
]
```
...
### 4.4 查詢模式選擇器（2026/08/06 新增，取代原本的雙模式判斷）

篩選列新增「查詢模式」單選標籤群（`#modeGroup`，`setQueryMode(mode)`），三選一：`date`（日期瀏覽，預設）、`mobile`（電話號碼）、`userid`（USER_ID）。三者互斥，同一時間只有選中模式對應的輸入框可用（`disabled` 灰階，非選中模式的輸入框即使殘留舊文字也不會被查詢邏輯讀取）。

- 原本（2026/08/05 版）用「電話輸入框是否有值」自動判斷模式，使用者 2026/08/06 要求改成「只能擇一」的明確模式選擇，且不要「輸入時自動清空對方欄位」或「兩欄位同時生效（AND 疊加）」這兩種替代做法
- 切換模式：點快速日期按鈕（1/3/7天前）、按「重置」都會強制切回 `date` 模式並清空電話／USER_ID 輸入框
- `applyFilter()` 依 `queryMode` 分派：`mobile`/`userid` 模式查 `ALL_INDEX`（比對 `m`/`u` 欄位，`includes` 部分比對，忽略日期區間）；`date` 模式查 `BLACKLIST_DATA`（依 `UPDATE_TIME` 篩選，邏輯不變）

### 4.5 USER_ID 查詢（2026/08/06 新增）

新增獨立輸入框 `#useridSearch`，作為第三種查詢模式（見 §4.4）。查詢範圍與電話搜尋相同——`ALL_INDEX`（全部 40 萬餘會員，不分黑名單狀態），比對 `u`（USER_ID）欄位，`includes` 部分字串比對，忽略日期區間。與電話搜尋共用同一個 `mapAllIndexRow()` 轉換函式與排序邏輯（依 `ut` 新到舊）。

### 4.6 MAJOR 標籤（同門號對應多個 USER_ID，2026/08/06 新增）

Generator 端計算：以 `MOBILE_head + MOBILE`（國碼+完整號碼）為 key，對全體 `ALL_INDEX`（40 萬餘會員）分組，找出同一門號對應 `MAJOR_THRESHOLD` 個以上不同 `USER_ID` 的帳號，一律標記 `wt: 'MAJOR'`（欄位省略＝非 MAJOR，節省檔案大小）。`BLACKLIST_DATA` 裡的對應帳號也同步標記（用同一份 `majorUids` 查表）。前端在結果表格 USER_ID 欄位文字旁加一個小紫色「MAJOR」標籤（不佔額外欄位），三種查詢模式（電話／USER_ID／日期瀏覽）查到的列都會顯示。
...
### 4.7 UPDATE_TIME 欄位移除（2026/08/06）

結果表格移除 UPDATE_TIME 顯示欄位與其排序功能（`sortTable()` 的可排序欄位清單同步移除 `'ut'`），欄位總數從 9 欄降為 8 欄（`colspan` 同步從 9 改 8）。**僅移除表格顯示**，篩選列的「異動日期（UPDATE_TIME）」日期區間選擇器與 1/3/7 天前快速按鈕維持不變，`BLACKLIST_DATA`/`ALL_INDEX` 資料本身仍保留 `ut` 欄位供日期篩選與預設排序（依 `ut` 新到舊）使用，只是不再顯示成表格欄位。
