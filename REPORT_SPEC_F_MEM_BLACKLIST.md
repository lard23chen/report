...
| 資料範圍 | **僅嵌入近 90 天內** `CREATE_TIME` 且 `MEMO0='Y'` 的資料（見 §2 為何限制範圍） |
| 負責人 | 陳俊良 |
| 主要目的 | 查詢近期被標記為黑名單（`MEMO0='Y'`）的會員帳號，依「加入黑名單日期」區間與 `USER_ID` 篩選 |
...
2026/08/04 與使用者確認後改為：只嵌入近 90 天（`CREATE_TIME >= 今日 - 90天`）的 `MEMO0='Y'` 資料，實測約 25,000+ 筆、檔案 ~4.6MB。**查更早期資料需另行以 MongoDB 查詢，本報表查不到**（頁面 header 有 warning 提示此限制）。
...
```js
{ total: 25272, minDate: "2026-05-06", maxDate: "2026-08-03" }
```

- `minDate` / `maxDate`：本次嵌入資料中實際存在的 `CREATE_TIME` 日期範圍（依台北時間），用於 flatpickr 的 `minDate`/`maxDate` 限制與「N天前」按鈕的錨點
...
| 條件 | 說明 |
|------|------|
| 日期區間 | 依 `CREATE_TIME`（加入黑名單時間，非 `UPDATE_TIME`）篩選，flatpickr 雙欄位 `#dtFrom`/`#dtTo`，`dateFormat:'Y-m-d'` |
...
欄位：USER_ID、EMAIL、MOBILE_head、CREATE_TIME、UPDATE_TIME、CREATE_USER、UPDATE_USER。點欄位 header 可排序（`sortTable()`），預設依 CREATE_TIME 由新到舊。表格上方顯示「符合條件：N 筆」。
...
|------|-------------|-------|------------|------|
| 近 90 天黑名單資料 | `MONGODB_URI_QWARE` | `QwareAi` | `Qware_MEM_BlackList_202608` | `MEMO0:"Y"`, `CREATE_TIME >= now - 90天` |

主要欄位：`USER_ID`、`EMAIL`、`MOBILE_head`、`CREATE_TIME`、`UPDATE_TIME`、`CREATE_USER`、`UPDATE_USER`、`MEMO0`（`Y`/`N`/`4`/`null`，只有 `Y` 才是本報表要查的黑名單標記）。collection 只有 `_id` 索引，全表 375 萬筆，`find()` 前務必先用 `MEMO0`+`CREATE_TIME` 縮小範圍，避免全表掃描。
...
---
*建立日期：2026/08/04｜使用者需求為「用 CREATE_TIME + USER_ID 查詢 MEMO0='Y' 的黑名單資料」；因全量 85,485 筆嵌入會產生 ~15MB 異常檔案，與使用者確認後改為只嵌入近 90 天（~25,000 筆，~4.6MB），預設檢視為近 3 天*
