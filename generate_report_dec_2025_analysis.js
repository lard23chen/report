...
async function generateReport() {
    try {
        ...
        const data = await collection.find({ "交�??��?": { $regex: "^2025-12" } }).toArray();
        console.log(`Fetched ${data.length} records.`);

        // Calculate Date Range
        const dates = data
            .map(d => d['交�??��?'] ? new Date(d['交�??��?'].split(' ')[0]) : null)
            .filter(d => d !== null);
        ...
        if (dates.length > 0) {
            ...
            if (startYear === endYear && startMonth === endMonth) {
                dateTitle = `${startYear}�?{startMonth}???��??�表`;
            } else if (startYear === endYear) {
                dateTitle = `${startYear}�?{startMonth}??${endMonth}???��??�表`;
            } else {
                dateTitle = `${startYear}�?{startMonth}??- ${endYear}�?{endMonth}???��??�表`;
            }
        }
...
<head>
    ...
    <title>ibon?�票系統 ${dateTitle} - ${reportTime}</title>
...
<div class="container">
    ...
    <div class="stats-grid">
        <div class="card">
            <h3>總�???(Total Revenue)</h3>
            <div class="value" id="val-revenue">--</div>
            <div class="sub">?�正常交??/div>
        </div>
        <div class="card">
            <h3>客單??(AOV)</h3>
            <div class="value" id="val-aov">--</div>
            <div class="sub">?�收 / 訂單筆數</div>
        </div>
        <div class="card">
            <h3>總交?�張??(Total Tickets)</h3>
            <div class="value" id="val-tickets">--</div>
            <div class="sub">總售?�票?�數??/div>
        </div>
        <div class="card">
            <h3>總�??��???(Distinct Orders)</h3>
            <div class="value" id="val-orders">--</div>
            <div class="sub">不�?複�??�編??/div>
        </div>
        <div class="card">
            <h3>總退票張??(Total Refunded Tickets)</h3>
            <div class="value" id="val-refund-tickets">--</div>
            <div class="sub" style="color: #c62828;">已退�??�票�???/div>
        </div>
        <div class="card">
            <h3>總退票�?�?(Refunded Amt)</h3>
            <div class="value" id="val-refunded-amount">--</div>
            <div class="sub">?�票票?��?總價</div>
        </div>
        <div class="card">
            <h3>?�票�?續費 (Refund Fees)</h3>
            <div class="value" id="val-refund-fees">--</div>
            <div class="sub">?�票產?��??�收</div>
        </div>
    </div>
    ...
    <div class="table-container">
        <h3 style="color: var(--text-secondary); border-bottom: 2px solid var(--accent-color); padding-bottom: 10px;">?? ?�售?��? Top 5 (By Revenue)</h3>
        <table id="topEventsTable">
            <thead>
                <tr>
                    ...
                    <th class="text-right">佔�? (Share)</th>
                </tr>
            </thead>
            <tbody></tbody>
        </table>
    ...
    <div class="table-container">
        <h3 style="color: #c62828; border-bottom: 2px solid #c62828; padding-bottom: 10px;">?? ?�票�?�?Top 5 (By Refund Amount)</h3>
    ...
    <div class="table-container">
        <h3 style="color: var(--text-secondary); border-bottom: 2px solid var(--accent-color); padding-bottom: 10px;">?�� 付款?��??��? (Payment Methods)</h3>
    ...
    <div class="table-container">
        <h3 style="color: var(--text-secondary); border-bottom: 2px solid var(--accent-color); padding-bottom: 10px;">?�� ?�票?��??��? (Ticket Types: Paper vs Electronic)</h3>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="table-container" style="margin-top:20px;">
                <h3>?�售點�???(Sales Points)</h3>
                <table id="modalSalesPointTable">
                    <thead>
                        <tr><th>?�售�?/th><th>張數 (Tickets)</th><th>?�收 (Revenue)</th><th class="text-right">佔�? (Share)</th></tr>
                    </thead>
                    <tbody></tbody>
                </table>
            </div>
            ...
            <div class="table-container" style="margin-top:20px;">
                ...
                <table id="modalPriceTable">
                    <thead>
                        <tr><th>票價</th><th>張數</th><th>?�收</th><th class="text-right">佔�?</th></tr>
                    </thead>
                    <tbody></tbody>
                </table>
            </div>
            ...
            <div class="table-container" style="margin-top:20px;">
                <h3 style="color:#c62828;">?�票�???(Refund Analysis)</h3>
                <table id="modalRefundTable">
                    <thead>
                        <tr><th>?�票�?�?(Reason)</th><th>張數 (Tickets)</th><th>?��?�?(Fee)</th></tr>
                    </thead>
                    <tbody></tbody>
                </table>
            </div>
...
<script>
    ...
    function analyzeEvent(eventName) {
        ...
        // Filter Data (ALL)
        const allEventData = dbData.filter(d => d['節???��??�稱'] === eventName);
        const eventData = allEventData.filter(d => d['?�??] === '�?��'); // Valid Sales
        const refundData = allEventData.filter(d => d['?�??] === '已退�? || d['?�??] === '?��? || (d['?��?�?] && d['?��?�?] > 0)); 
        
        // 1. Basic Stats
        const revenue = eventData.reduce((acc, cur) => acc + (cur['?�價'] || 0), 0);
        const tickets = eventData.length;
        const totalRefAmount = allEventData.reduce((acc, cur) => acc + (cur['?��?�?] || 0), 0);
        
        const uniqueOrders = new Set();
        eventData.forEach(o => {
            if(o['訂單編�?']) uniqueOrders.add(o['訂單編�?'].split('_')[0]);
        });
        ...
        priceList.forEach(p => {
             ...
             tr.innerHTML = \`
                <td>\${p.price === 0 ? '?�費/?��?' : '$'+p.price.toLocaleString()}</td>
        ...
        modalPriceChart = new Chart(document.getElementById('modalPriceChart'), {
            ...
            options: {
                ...
                plugins: { 
                    title: { display: true, text: '票價?��? (Tickets by Price)' },
        ...
        eventData.forEach(o => {
            if(!o['交�??��?']) return;
            const date = o['交�??��?'].split(' ')[0];
            dailyStats[date] = (dailyStats[date] || 0) + (o['?�價'] || 0);
        });
        ...
        modalTrendChart = new Chart(document.getElementById('modalTrendChart'), {
            ...
            options: {
                 ...
                 maintainAspectRatio: false,
                 plugins: { 
                    title: { display: true, text: '?�售趨勢 (Sales Trend)' },
        ...
        if(refundList.length === 0) {
            rfTbody.innerHTML = '<tr><td colspan="3" style="text-align:center; color:#999;">?�退票�???/td></tr>';
        } else {
    ...
    function init() {
        ...
        // Filter valid orders
        const validOrders = dbData.filter(d => d['?�??] === '�?��');
        
        // 1. Overall Stats Calculation
        const totalRevenue = validOrders.reduce((acc, cur) => acc + (cur['?�價'] || 0), 0);
        
        // Refund Calculations
        const refundDocs = dbData.filter(d => d['?�??] === '已退�? || d['?�??] === '?��?);
        const totalRefundedValue = refundDocs.reduce((acc, cur) => acc + (cur['實退?��?'] || 0), 0);
        const totalRefundFees = dbData.reduce((acc, cur) => acc + (cur['?��?�?] || 0), 0);
        
        const totalTickets = validOrders.length; 
        const totalRefundedTickets = dbData.filter(d => d['?�??] === '已退�? || d['?�??] === '?��? || (d['?��?�?] && d['?��?�?] > 0)).length; 

        // Count distinct orders
        const uniqueOrdersSet = new Set();
        validOrders.forEach(o => {
            if(o['訂單編�?']) {
                const baseOrder = o['訂單編�?'].split('_')[0];
        ...
        validOrders.forEach(item => {
            const method = item['付款?��?'] || 'Unknown';
            const price = item['?�價'] || 0;
            const orderId = item['訂單編�?'] ? item['訂單編�?'].split('_')[0] : 'unknown';
        ...
        const typeStats = {
            '紙票 (Paper/Picked Up)': { orders: new Set(), tickets: 0, revenue: 0 },
            '?��?�??��? (E-Ticket/Not Printed)': { orders: new Set(), tickets: 0, revenue: 0 },
        ...
        validOrders.forEach(item => {
            ...
            let typeKey = '?��? (Other)';
            if (method === '已�?') typeKey = '紙票 (Paper/Picked Up)';
            else if (method === '?��???) typeKey = '?��?�??��? (E-Ticket/Not Printed)';
        ...
        const refundStats = {};
        const refundRecords = dbData.filter(d => d['?�??] === '已退�? || d['?�??] === '?��? || (d['?��?�?] && d['?��?�?] > 0)); 
        ...
        if(topRefunds.length === 0) {
             topRefundTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">?�退票�???/td></tr>';
        } else {
        ...
        validOrders.forEach(item => {
            if (!item['交�??��?']) return;
            const date = item['交�??��?'].split(' ')[0];
            const price = item['?�價'] || 0;
            const name = item['節???��??�稱'] || '?�知';
            const orderId = item['訂單編�?'] ? item['訂單編�?'].split('_')[0] : 'u';
        ...
        new Chart(document.getElementById('trendChart'), {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: { 
                    title: { display: true, text: '?�收趨勢??(Sales Trend)' },
                    annotation: {
        ...
        refundRecords.forEach(item => {
             ...
             if (!dateStr || dateStr === '-') dateStr = item['交�??��?'];
             if (!dateStr) return;
        ...
        new Chart(document.getElementById('refundTrendChart'), {
            ...
            options: {
                ...
                maintainAspectRatio: false,
                plugins: { 
                    title: { display: true, text: '每日?�票走??(Daily Refund Trend)' },
                    tooltip: {
                         callbacks: {
