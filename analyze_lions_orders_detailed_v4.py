...
try:
    ...
    for line in lines[1:]:
        ...
        if times:
            ...
            # OrderNo is usually the last part (lengthy string starting with A26...)
            order_no = parts[-1] if len(parts) > 6 else "Unknown"
    ...
    # 1. Core Metrics
    total_tickets = len(df) # Total rows
    total_orders = df['OrderNo'].nunique() # Unique Order IDs
    unique_members = df['MemberNo'].nunique()

    # 2. Minute Stats
    df['Minute'] = df['TransTime'].str.extract(r'(\d{2}:\d{2})')
    # Minute analysis based on unique orders
    ...
    # 4. Heavy Buyers (by Unique Orders)
    ...
    <head>
        <meta charset="UTF-8">
        <title>統一獅 3/9 18:00-19:00 訂單與張數深度分析</title>
        <style>
    ...
    <body>
    <header>
        <h1>🦁 統一獅 3/9 開賣首小時 訂單 vs 張數 分析</h1>
        <div style="color:var(--muted)">資料來源：Lions_Orders_March9_1800.txt</div>
    </header>
    <div class="container">
