...
- **主要目的**：追蹤「售票網_正式環境」（AccountId `822996750037`）每月 AWS 雲端費用，呈現月費用趨勢、帳單各項加減費用（Enterprise Support 費、優惠折扣、維運服務費）的明細拆解，以及各 AWS 服務的成本排行。
- **系統別**：D系統（「售票網」為 D 系統的正式環境，與 `D_GA_Funnel_202606_Report.html`／`D_GA_ClickData_Webb_202606_Report.html` 等既有 D 系統報表同一系統，目錄與 `report_index.html` 卡片皆標記 `sys-badge sys-d` / `D系統`）。
...
| Row 1~58 | 每個 AWS 服務一組（值列 + 中文說明列，說明列數值欄位皆為空；`parseServiceRows()` 用「該列任一月份欄位為 number」判斷值列，藉此跳過空白列與中文說明列，不依賴固定 row index） |
| Row 59 | `服務合計USD` |
| Row 60 | `結算匯率` |
| Row 61 | `服務合計USD-->NTD` |
...
本報表**未使用**明細分頁（服務成本排行也是從 `總表` 各服務列取得，見 §4.2b）。明細分頁含每個 AWS 服務依地區/用量拆分的完整帳單行項目，以及含稅金額（`應繳總金額NTD(含稅)`，`總表` 沒有此列）。若未來需要「服務內地區/用量拆分」或「含稅金額」，資料來源在此，欄位為 `A`=項目說明、`B`=unit price、`C`=Usage、`D`=sub-Total（USD）。
...
- **表格欄位**：排名 / AWS 服務（英文名 + 中文說明）/ 每月金額欄位（依 `yearMonths` 動態展開，如 `2026/01`~`2026/06`）/ 年度累計
  ...
  - **每月金額下方附 MoM 漲跌幅**（`changeBadge(cur, prev)`）：與同一份報表其他表格語意一致，費用上升=壞=紅 `▲`、下降=好=綠 `▼`
    - 上月為 0 且當月也是 0：不顯示（避免 `0% → 0%` 的雜訊）
    - 上月為 0、當月 > 0：顯示「新增」（無法計算百分比，紅字）
    - 當月為 0、上月 > 0：正常計算得到 `▼ 100%`
    - 第一個月（`2026/01`）沒有上月資料可比較，不顯示漲跌幅
...
*2026/07/31：§4.2b 逐月矩陣表格每格加上 MoM 漲跌幅（`changeBadge()`），紅漲綠跌，涵蓋「新增」（上月 0）與「歸零」（當月 0）邊界情況*
