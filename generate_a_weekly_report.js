...
// 週報區間：上週四 ~ 本週三（最近一個已過完的週三往前推 6 天）
function getWeekRange() {
    const end = new Date();
    end.setHours(0, 0, 0, 0);
    // 往回找最近的週三（getDay() === 3）；當天若是週三則往回找上一個週三
    do { end.setDate(end.getDate() - 1); } while (end.getDay() !== 3);
    const start = new Date(end);
    start.setDate(start.getDate() - 6); // 週四
...
async function generateWeeklyReport() {
    try {
        ...
        data.forEach(d => {
            ...
            if (d['退票時間'] && d['退票時間'].includes('/')) {
                d['退票時間'] = d['退票時間'].replace(/\//g, '-');
            }
        });

        const week = getWeekRange();
        console.log(`Weekly range: ${week.start} ~ ${week.end}`);

        // 排除訂單編號 B 開頭的訂單
        const beforeExclude = data.length;
        data = data.filter(d => !(d['訂單編號'] && String(d['訂單編號'])[0] === 'B'));
        console.log(`Excluded ${beforeExclude - data.length} B-prefix rows.`);

        // 週期過濾：正常交易看交易時間；退票看退票時間（無值 fallback 交易時間）
        ...
        data = data.filter(d => {
            if (d['狀態'] === '已退票' || d['狀態'] === '退票') {
                let refDate = d['退票時間'];
                if (!refDate || refDate === '-') refDate = d['交易時間'];
                return inRange(refDate);
            }
            return inRange(d['交易時間']);
        });
        ...
        const dailyMap = {};
        data.forEach(item => {
            let dateStr = item['交易時間'];
            if ((item['狀態'] === '已退票' || item['狀態'] === '退票')) {
                if (item['退票時間'] && item['退票時間'] !== '-') dateStr = item['退票時間'];
            }
            if (!dateStr) return;
        ...
        // Date range for title (fixed to the weekly window)
        const fmtDate = (d) => `${d.getFullYear()}年${String(d.getMonth()+1).padStart(2,'0')}月${String(d.getDate()).padStart(2,'0')}日`;
        const dateTitle = `${fmtDate(week.startDate)} - ${fmtDate(week.endDate)} 週報分析報表 (A系統)`;
