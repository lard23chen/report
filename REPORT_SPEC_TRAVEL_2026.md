...
**⚠️ 尚未確認飯店時：保持 `<tbody></tbody>` 空白即可，切勿插入 `colspan` 佔位列。**

> **原因**：`<tr><td colspan="8">...</td></tr>` 只有 1 個 `<td>`，當使用者在編輯模式下點擊該列的 ✏️ 按鈕時，`editRow()` 只能看到 1 個欄位，導致表單破損、資料無法正常輸入。新增飯店資料請一律使用「＋ 新增一行」按鈕。
...
| Tab | 狀態 |
|-----|------|
| 2027 Trip 1–2 | 有結構，tbody 空白，可在編輯模式下新增 |
| 2026 Trip 1, 2, 4, 5 | 已填入完整資料 |
| 2026 Trip 3（聖誕跨年） | 有結構，待補資料（空白 tbody） |
| 2025 Trip 1–5 | 已填入完整資料 |
| 2024 T1–T7 | 有結構，待補資料（空白 tbody） |
| 2023 T1–T4 | 有結構，待補資料（空白 tbody） |
| 2022 T1 | 已填入完整資料 |
...
### 5. 修正 2027 飯店無法新增 bug — 2026-06-09
*   **根因**：2027 Trip 1–2 的 hotel `<tbody>` 各有一個 `<tr><td colspan="8">— 飯店資料待補 —</td></tr>` 佔位列。在編輯模式下點擊該列 ✏️ 按鈕時，`editRow()` 只找到 1 個 `<td>`（colspan 列），導致表單僅顯示 1 個欄位，完全無法輸入完整飯店資料。
*   **修正**：移除 2027 Trip 1、Trip 2 hotel `<tbody>` 內的 colspan 佔位 `<tr>`，改為空白 tbody。
*   **規範更新**：**禁止使用 colspan 佔位列**；尚未有飯店資料時 tbody 應保持空白（詳見 §5.1）。
*   **影響範圍**：現有 localStorage / MongoDB 中已存的飯店資料不受影響（data-eid 未變動）。
...
### 7. 修正日期星期註記重複插入 bug (Weekday Annotation Corruption) — 2026/07/07

*   **症狀**：trip 日期顯示為 `2027/02/0(日)2(二) – 2027/02/1(一)5(日)` 之類的錯亂字串。
*   **根因**：`addDayOfWeek()` 原以 `/(\d{4})\/(\d{1,2})\/(\d{1,2})(?!\()/g` 附加星期，靠負向預查跳過已註記日期。但對已含註記的文字（HTML 寫死或 tripEdits 存回的值），`\d{1,2}` 比對日「02」後遇 `(` 失敗會**回溯成單一數字「0」**，下一字元非 `(` 使比對成立，於是在日期中間再插入一次星期。
*   **修正**（`addDayOfWeek()`，約 2222 行起）：
    1.  先移除「星期註記後面直接接數字」的無效註記（修復已腐蝕字串，包含存於 localStorage / MongoDB tripEdits 的值）。
    2.  再剝除完整日期後的既有星期註記並**全部重新計算**（canonical；一併自動修正人工填錯的星期，如 2027/02/15 實為週一、原寫死為（日））。
    3.  附加註記的預查改為 `(?![\d(])`，杜絕回溯部分比對。
    4.  `.hotel-dates`（M/D 格式）分支同步改為剝除既有註記後重算。
*   **冪等性**：函式重複執行結果不變，tripEdits 存回已註記字串亦不會再腐蝕。
