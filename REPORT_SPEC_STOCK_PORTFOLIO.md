...
*   **篩選小計 (Footer Totals)**：表格底部自動加總當前篩選結果的總支出與總收入，並在損益欄顯示淨損益金額。
    *   **獲利百分比標註**：當 `fNet > 0` 且 `fSpend > 0` 時，損益金額後方自動附加綠色小字 `(+X.XX%)`。
    *   計算式：`(fNet / fSpend) × 100`（報酬率 = 獲利 ÷ 總投入支出）。
    *   同步顯示於表格上方的 `filter-count` 統計行與表格底部的 `tx-foot` 小計列。
    *   虧損（`fNet ≤ 0`）或無支出（`fSpend = 0`）時不顯示百分比，外觀維持原樣。
