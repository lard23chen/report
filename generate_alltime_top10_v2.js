...
async function main() {
    ...
    try {
        ...
        // ============ Step 1: Get Top 10 ============
        console.log("Running Top 10 aggregation...");
        const top10Raw = await coll.aggregate([
            { $match: { name: "page_view", bu: "D", content_name: { $ne: null, $ne: "" }, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// } } },
        ...
        // ============ Step 2: Get total views for percentage ============
        ...
        // ============ Step 3: For each program, get detailed stats ============
        ...
        for (let idx = 0; idx < top10.length; idx++) {
            ...
            console.log(`[${idx+1}/10] Analyzing: ${prog.name}...`);

            // 3a: Monthly breakdown
            const monthlyData = await coll.aggregate([
                { $match: { name: "page_view", bu: "D", content_name: prog.name, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// }, time: { $exists: true, $ne: null } } },
                { $group: { _id: { $dateToString: { format: "%Y-%m", date: "$time" } }, views: { $sum: 1 } } },
                { $sort: { _id: 1 } }
            ]).toArray();

            // 3b: Daily breakdown (for chart)
            const dailyData = await coll.aggregate([
                { $match: { name: "page_view", bu: "D", content_name: prog.name, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// }, time: { $exists: true, $ne: null } } },
                { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$time" } }, views: { $sum: 1 } } },
                { $sort: { _id: 1 } }
            ]).toArray();

            // 3c: Device breakdown from user_agent
            const deviceData = await coll.aggregate([
                { $match: { name: "page_view", bu: "D", content_name: prog.name, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// } } },
                { $project: { device: { $cond: [ { $regexMatch: { input: { $ifNull: ["$user_agent", ""] }, regex: /Mobile|Android|iPhone|iPad/ } }, "Mobile", "Desktop" ] } } },
                { $group: { _id: "$device", count: { $sum: 1 } } },
                { $sort: { count: -1 } }
            ]).toArray();

            // 3d: Hourly distribution
            const hourlyData = await coll.aggregate([
                { $match: { name: "page_view", bu: "D", content_name: prog.name, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// }, time: { $exists: true, $ne: null } } },
                { $group: { _id: { $hour: "$time" }, views: { $sum: 1 } } },
                { $sort: { _id: 1 } }
            ]).toArray();

            // 3e: Unique visitors (fp_id)
            const uniqueVisitors = await coll.aggregate([
                { $match: { name: "page_view", bu: "D", content_name: prog.name, canonical_url: { $regex: /^https:\/\/ticket\.ibon\.com\.tw\// } } },
                { $group: { _id: "$fp_id" } },
                { $count: "uniqueCount" }
            ]).toArray();
            const uvCount = uniqueVisitors[0]?.uniqueCount || 0;

            // Build detail HTML
            const monthLabels = monthlyData.map(d => d._id);
            const monthValues = monthlyData.map(d => d.views);

            const dailyLabels = dailyData.map(d => d._id);
            const dailyValues = dailyData.map(d => d.views);

            const deviceLabels = deviceData.map(d => d._id || 'Unknown');
            ...
            // Monthly table rows (newest first)
            ...
            const monthTableRows = [...monthlyData].reverse().map(m => {
                ...
                return `<tr><td style="color:#94a3b8;">${m._id}</td><td style="text-align:right;color:#f8fafc;font-weight:500;">${m.views.toLocaleString()}</td><td style="text-align:right;color:#a78bfa;">${pct}%</td></tr>`;
            }).join('');

            // Device table rows
            const deviceTableRows = deviceData.map(d => {
                const pct = deviceTotal > 0 ? ((d.count / deviceTotal) * 100).toFixed(1) : '0.0';
                return `<tr><td style="color:#94a3b8;">${d._id || 'Unknown'}</td><td style="text-align:right;color:#f8fafc;font-weight:500;">${d.count.toLocaleString()}</td><td style="text-align:right;color:#34d399;">${pct}%</td></tr>`;
            }).join('');
            ...
            const peakDay = dailyLabels[peakIdx] || '-';
            const peakViews = dailyValues[peakIdx] || 0;
...
<body>
    ...
    <script>
        ...
        // ============ Step 4: Regenerate main Top 10 report ============
        console.log("Building main Top 10 report...");
