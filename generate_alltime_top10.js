...
async function main() {
    ...
    try {
        ...
        console.log("Running aggregation pipeline for Top 10 all-time stats...");
        const pipeline = [
            {
                $match: {
                    name: "page_view",
        ...
        console.log(`Aggregation complete. Found top 10 results.`);

        const top10 = rawResults.map((r, index) => {
            const ids = new Set();
            if (r.attribution_ids && Array.isArray(r.attribution_ids)) {
                r.attribution_ids.forEach(id => {
                    if (id && id.trim() !== '' && id !== 'null' && id !== 'undefined') ids.add(id);
                });
            }
            return {
                name: r._id,
...
<body>
    ...
    <script>
        ...
        const overallTotalViews = totalViewsResult.length > 0 ? totalViewsResult[0].totalViews : totalTop10Views;


        const tableRows = top10.map((item, index) => {
            const rankClass = index === 0 ? 'rank-1' : index === 1 ? 'rank-2' : index === 2 ? 'rank-3' : '';
            const percentage = ((item.totalViews / overallTotalViews) * 100).toFixed(2);
...
<body>
        ...
        <div class="header">
            ...
            <div style="display: flex; gap: 15px; align-items: stretch;">
                <div class="stat-card">
                    ...
                    <div class="value">${overallTotalViews.toLocaleString()}</div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const outPath = path.join(__dirname, 'A_DMP_PageView_Report_AllTime_Top10.html');
