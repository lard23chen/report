...
<head>
    ...
    <style>
        ...
        th { text-align: center; padding: 12px; color: #cbd5e1; border-bottom: 2px solid rgba(255,255,255,0.1); background: rgba(15, 23, 42, 0.85); position: sticky; top: 0; backdrop-filter: blur(8px); z-index: 10; }
        ...
        .select-month { background: #1e293b; color: #60a5fa; border: 1px solid #3b82f6; padding: 10px 20px; border-radius: 999px; cursor: pointer; }
...
<body>
    <div class="container">
        <div class="header">
            <div><h1>每日雲端報表與費用分析</h1><div style="color: #64748b; font-size: 0.9rem;">Source: MongoDB</div></div>
            <select id="monthFilter" class="select-month"><option value="all">所有月份 (All Time)</option>${filterOptions}</select>
        </div>
        <div class="card">
            <h3 style="color: #34d399;">每日明細與活動細節</h3>
            <div class="table-wrapper">
                <table>
                    <thead><tr><th>日期</th><th>A系統</th><th>D系統(AWS)</th><th>E系統</th><th>共用</th><th>會員</th><th>總計[未稅]</th><th>主要活動與備註</th></tr></thead>
                    <tbody id="tableBody"></tbody>
                </table>
            </div>
        </div>
        <div class="card">
            <h3 style="color: #a78bfa;">費用趨勢分析圖</h3>
    ...
    <script>
        ...
        function renderTable(dat) {
            document.getElementById('tableBody').innerHTML = dat.map(d => \`
                <tr>
                    <td>\${d.date}</td>
