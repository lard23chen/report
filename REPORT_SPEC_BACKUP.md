...
| # | 風險 | 優先度 | 狀態 |
|---|------|--------|------|
| 1 | **MongoDB 連線字串(含帳密)硬編碼在 235 支 js 檔（237 處）+ 4 個 MD 文檔內,且 repo 為公開**——3 組帳密已曝光（QwareDashBoard×2 cluster、lard23 個人） | **高** | **程式碼已於 2026/07/13 全數改讀 `.env`**（`MONGODB_URI_QWARE` / `MONGODB_URI_DMP` / `MONGODB_URI_PERSONAL`）。lard23:歷史中曝光的舊密碼已失效（風險解除;本機 `.env` 待與 Atlas 新密碼對齊,影響 server.js 與投資/保險工具）。**QwareDashBoard:曝光密碼仍有效,用戶 2026/07/13 決定暫不輪換——風險保留,強烈建議至少設 Atlas Network Access IP 白名單作為補償控制** |
