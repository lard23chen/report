...
async function generateReportForMonth(targetMonth) {
    try {
        ...
        const data = await collection.find({ "交易時間": { $regex: dateRegex } }).toArray();
        console.log(`Fetched ${data.length} records for ${targetMonth}.`);
        ...
        if (dates.length > 0) {
            ...
            // Force the title to match the target month if consistent, to avoid small date drift issues in title
            // But relying on data is usually fine. Let's stick to the targetMonth for the main part to be safe.
            const [tYear, tMonth] = targetMonth.split('-');
            fileTitle = `${tYear}年${tMonth}月 分析報表`;
            dateTitle = `${tYear}年${tMonth}月 分析報表 (A系統)`;
        }
        ...
        // HTML Template (Based on Jan 2026 Comprehensive Report)
        const htmlContent = `
<!DOCTYPE html>
<html lang="zh-TW">
<head>
...
<script>
    ...
    function init() {
        ...
        dates.forEach((date, index) => {
            // Lower threshold for older months if needed, or keep 6000000
            if (dailySales[index] > 3000000) { // Adjusted threshold for general purpose
                // Find max product
...
" onmouseover="this.style.transform='translateY(-3px)'; this.style.boxShadow='0 6px 20px rgba(216, 27, 96, 0.5)';" 
   onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 4px 15px rgba(216, 27, 96, 0.4)';">
    ...
    回首頁
        ...
        console.error(`Error generating report for ${targetMonth}:`, e);
    }
}
...
async function run() {
    try {
        ...
        // Generate for Oct and Nov 2025
        await generateReportForMonth('2025-11');
        await generateReportForMonth('2025-10');
