...
async function generateReport() {
    try {
        ...
        console.log("Fetching and aggregating January 2026 data...");
        ...
        // Replace Node.js template placeholders with actual January 2026 values
        const reportTime = new Date().toLocaleString('zh-TW');
        const dateTitle = "2026年01月 分析報表 (A系統)";
        let logoBase64 = '';
        try {
    ...
    window.addEventListener('load', function() {
        const s = summaryData;
        document.title = "ibon售票系統 2026年01月 分析報表 (A系統)";
        const h1 = document.querySelector('h1');
        if(h1) h1.innerText = "ibon售票系統 2026年01月 分析報表 (A系統)";
...
</body></html>`.replace(/2026年02月/g, "2026年01月");

        const fileName = `A_Qware_Revenue_Report_2026年01月_分析報表.html`;
