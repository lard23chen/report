...
async function generateReport() {
    try {
        ...
        console.log("Fetching and aggregating May 2026 data...");
        ...
        // Replace Node.js template placeholders with actual April 2026 values
        const reportTime = new Date().toLocaleString('zh-TW');
        const dateTitle = "2026年05月 分析報表 (A系統)";
        let logoBase64 = '';
        try {
    ...
    window.addEventListener('load', function() {
        const s = summaryData;
        document.title = "ibon售票系統 2026年04月 分析報表 (A系統)";
        ...
        if(h1) h1.innerText = "ibon售票系統 2026年04月 分析報表 (A系統)";
...
</body></html>`.replace(/2026年02月/g, "2026年05月");

        const fileName = `A_Qware_Revenue_Report_2026年05月_分析報表.html`;
        fs.writeFileSync(path.join(__dirname, fileName), '\ufeff' + finalHtml);
        console.log(`May 2026 report generated: ${fileName}`);
