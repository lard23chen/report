...
**`<tfoot id="detailTfoot">` 總計列**（每次 renderDetailTbody 呼叫時動態更新）：
...
| 股數 | — |
| 均價 | — |
| 金額 | 買 - $X（紅）/ 賣 + $X（綠）/ 股利 $X（金黃）各一行 |
| 手續費 | 全部費用加總 |
| 淨收付 | 淨流量（= 賣出淨 + 股利淨 − 買進淨），正值綠/負值紅 |

> **淨收付計算**：買進淨 = Σ(amount + totalFee)；賣出淨 = Σ|amount − totalFee|；股利淨 = Σ(d.net)  
> 篩選條件變更時自動重新計算，反映當前篩選結果的小計。
