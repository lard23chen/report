...
**`renderTable()` 只顯示有 PV 數字的活動（2026/08/10 新增）**：`pv == null`（該活動在目前日期區間內查無瀏覽量，只有點擊或完全沒資料）的列直接從表格濾掉，不再顯示「PV 欄一整排「—」」的列——這是一份以 PV 為主的排行榜，沒有 PV 數字的列對這個表格沒有意義。⚠️ 這個過濾只影響**表格**；頁首「涵蓋活動數」KPI 卡（`updateKPI()`）仍然計算 `pv!=null||clicks!=null`（PV 或 Click 任一有值即算），兩者口徑不同是刻意的——KPI 卡回答「這個時間範圍內 D 系統總共觸及幾個活動」，表格回答「PV 排行榜裡有哪些活動」，所以 KPI 數字可能會比表格實際列數多，這不是 bug。
