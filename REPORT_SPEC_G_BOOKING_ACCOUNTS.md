...
- 與 `F_MEM_BlackList_Query_Report.html`（`REPORT_SPEC_F_MEM_BLACKLIST.md`）同屬會員風控查詢系列，但本報表是**獨立**報表，不共用資料、不互相連結查詢

---
*建立日期：2026/08/05｜使用者需求為「再查所有帳號有 booking 的紀錄 Qware_A_OrderTemp_log_202608」；詢問後確認要獨立新建一份「有訂票紀錄的帳號」清單/報表，而非併入 F 報表的既有查詢流程。採 `$group` 彙總（非逐筆明細）控制檔案大小，~12.4MB；前端另設 `RENDER_CAP=1000` 避免 14 萬列直接塞進 DOM 卡死頁面（初版沒設上限時實測互動幾乎凍結，加上限後恢復正常）*
