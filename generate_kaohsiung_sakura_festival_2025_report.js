...
async function generateReport() {
    try {
        ...
        const TARGET = '2025 7-ELEVEN高雄櫻花季 SAKURA FESTIVAL';
...
<head>
    ...
    <title>2025 7-ELEVEN 高雄櫻花季 SAKURA FESTIVAL - 專案分析報表</title>
    ...
    <style>
        ...
        header { margin-bottom: 40px; display: flex; justify-content: space-between; align-items: flex-end; background: linear-gradient(to right, #1a0a20, #2d1040); padding: 40px; border-radius: 24px; box-shadow: var(--shadow); border-bottom: 4px solid var(--accent-color); }
        .logo-text { font-size: 2.5rem; font-weight: 800; background: linear-gradient(135deg, #f472b6, #c084fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0; letter-spacing: -1px; }
        ...
        .card { background: var(--card-bg); border-radius: 20px; padding: 25px; box-shadow: var(--shadow); border: 1px solid rgba(244, 114, 182, 0.1); transition: all 0.3s; }
        ...
        .chart-card { background: var(--card-bg); border-radius: 24px; padding: 30px; box-shadow: var(--shadow); min-height: 400px; border: 1px solid rgba(244, 114, 182, 0.1); margin-bottom: 30px; }
        ...
        .peak-table-container { overflow-x: auto; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; background: rgba(15, 10, 20, 0.5); }
        ...
        .notice-box { background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.4); border-radius: 16px; padding: 16px 24px; margin-bottom: 30px; font-size: 0.95rem; color: #fca5a5; }
        .peak-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 25px; margin-bottom: 30px; }
    </style>
</head>
<body>
<div class="container">
    <header>
        <div>
            <div class="logo-text">🌸 2025 高雄櫻花季</div>
            <div class="logo-sub">7-ELEVEN 高雄櫻花季 SAKURA FESTIVAL 專案銷售分析報表</div>
        </div>
        <div class="meta">報表生成: ${reportTime}<br>銷售紀錄: ${data.length.toLocaleString()} 筆<br>節目代碼: B07ZKHEO</div>
    </header>

    <div class="notice-box">
        ⚠️ 本活動退票率偏高（退票 8,269 張），疑因天候因素大量辦理退票，淨銷售數據請以「正常」狀態為準。
    </div>
    ...
    <!-- Peak Analysis: 1/4 & 1/5 -->
    <div class="peak-grid">
        <div class="chart-card" style="min-height:0">
            <h3>開賣尖峰時段分析 (2025-01-04 12:00 – 13:00)</h3>
            <div class="peak-table-container">
                <table id="peakTable04">
        ...
        <div class="chart-card" style="min-height:0">
            <h3>開賣尖峰時段分析 (2025-01-05 12:00 – 13:00)</h3>
            <div class="peak-table-container">
                <table id="peakTable05">
                    <thead><tr><th>時間</th><th class="text-right">當分鐘張數</th><th class="text-right">累計張數</th><th class="text-right">信用卡</th><th class="text-right">ATM</th><th class="text-right">累計轉換率</th></tr></thead>
                    <tbody></tbody>
                </table>
            </div>
...
function buildPeakTable(tableId, peakDate, valid) {
    const peakStats = {};
    for (let m = 0; m < 60; m++) {
    ...
    Object.keys(peakStats).sort().forEach(k => {
        ...
        if (s.t > 0) tr.style.background = 'rgba(244,114,182,0.06)';
...
function init() {
    ...
    // Show Cards
    const shows = [
        { date: '2025-03-28', label: '3/28 (五) Day 1' },
        { date: '2025-03-29', label: '3/29 (六) Day 2' },
        { date: '2025-03-30', label: '3/30 (日) Day 3' }
    ];
    ...
    new Chart(document.getElementById('trendChart'), {
        ...
        data: { labels: sortedDays, datasets: [{ label: '張數', data: sortedDays.map(d => tStats[d]), backgroundColor: '#f472b6', borderRadius: 6 }] },
    ...
    const sessionShows = [
        { date: '2025-03-28', label: '3/28 (五) Day 1', color: '#f472b6' },
        { date: '2025-03-29', label: '3/29 (六) Day 2', color: '#c084fc' },
        { date: '2025-03-30', label: '3/30 (日) Day 3', color: '#fb923c' }
    ];
    ...
    new Chart(document.getElementById('sessionTrendChart'), {
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: { display: true, align: 'top', anchor: 'end', font: { size: 10, weight: '600' }, color: (ctx) => ctx.dataset.borderColor, formatter: (v) => v > 0 ? v.toLocaleString() : '' },
    ...
    new Chart(document.getElementById('genderChart'), {
        type: 'doughnut',
        data: { labels: Object.keys(gStats), datasets: [{ data: Object.values(gStats), backgroundColor: ['#f472b6','#c084fc','#94a3b8','#fb923c'] }] },
        plugins: [ChartDataLabels],
    ...
    new Chart(document.getElementById('ageChart'), {
        type: 'bar',
        plugins: [ChartDataLabels],
        data: { labels: ageLabels, datasets: [{ data: ageLabels.map(k => ageStats[k]), backgroundColor: '#c084fc', borderRadius: 6 }] },
    ...
    new Chart(document.getElementById('cityChart'), {
        type: 'bar',
        plugins: [ChartDataLabels],
        data: { labels: topCities.map(c=>c[0]), datasets: [{ data: topCities.map(c=>c[1]), backgroundColor: '#c084fc', borderRadius: 4 }] },
    ...
    // Peak tables for 1/4 and 1/5
    buildPeakTable('peakTable04', '2025-01-04', valid);
    buildPeakTable('peakTable05', '2025-01-05', valid);
}
init();
<\/script>
</body>
        ...
        const fileName = 'A_KaohsiungSakuraFestival_2025.html';
